data=read.csv("SA-discount.csv", header=T)
scalefactor=1000000
## add extra space to right margin of plot within frame
par(mar=c(5, 4, 4, 6) + 0.1)

## Plot first set of data and draw its axis
plot(data$discount, data$Extra, pch=2, axes=FALSE, ylim=c(10,20), xlab="", ylab="", 
...
plot(data$discount, data$Time, pch=4,  xlab="", ylab="", ylim=c(10,20), 
...
## Draw the time axis
axis(1,pretty(range(data$discount),10))
...
## Add Legend
legend("topleft",inset=.02,legend=c("Option value (mu)","Time (Year)"),
       text.col=c("blueviolet","coral1"),pch=c(2,4),col=c("blueviolet","coral1"),horiz=F,cex=0.8)