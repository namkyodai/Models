data=read.csv("SA-alpha.csv", header=T)
scalefactor=1000000
## add extra space to right margin of plot within frame
par(mar=c(5, 4, 4, 6) + 0.1)

## Plot first set of data and draw its axis
plot(data$alpha, data$Extra, pch=2, axes=FALSE, ylim=c(11,17), xlab="", ylab="", 
...
plot(data$alpha, data$Time, pch=4,  xlab="", ylab="", ylim=c(10,20), 
...
## Draw the time axis
axis(1,pretty(range(data$alpha),10))
mtext(expression(paste("Deterioration parameter of DS2  ", alpha)),side=1,col="black",line=2.5)  
