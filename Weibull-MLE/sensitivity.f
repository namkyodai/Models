...
!	call step1(alpha,m,c1A,c2A,r)
	...
	m1=2.484087137
!	in the case type A used for type A	
!	alpha1=3.44E-05
!	m1=2.144000053
...
!	This part is used for sensitivity analysis given the case type A replacing type C

	! When chaning the value of discount rate (Range from 0.001 to 0.1 - 100 values)
	...
	open(110, file=file110)
	open(120, file=file120)
	open(200, file=file200)
	open(210, file=file210)
	open(220, file=file220)
...
!		read(100,*) r1(ii), c1(ii), c2(ii) ! case 1, discount rate change
		read(110,*) r1(ii), c1(ii), c2(ii) ! case 2, social cost change
!		read(120,*) r1(ii), c1(ii), c2(ii)	!case 3, direct cost change
!		Print*, "value of discount", r1(ii)
!		Print*, "value of social cost", c1(ii)
!		Print*, "value of direct cost", c2(ii)


		!		Define directly the way to estimate LCC (similar way to subroutine but different
	! variable definition	
	...
	!	stop
				x1r=x2r
			x2r=exp(-alpha1*ttr**m1-r1(ii)*ttr)
			tpr=tpr+(x1r+x2r)*ddr/2.0
			JJJr=(c1(ii)+c2(ii)+lcc1(i))
     #	*(1.0-x2r-r1(ii)*tpr)+(c2(ii)+lcc1(i))*x2r
			ur(i) =JJJr
			ttr=ttr+ddr
!		Print*, u(i)
...
!		pRINT*, alpha, m
		...
		Minur=ur(1)
		do i=1, n
			if (Minur.GT.ur(i)) Minur=Ur(i)
		end do
	
		do i=1, n
		if (Ur(i).EQ.Minur) kr=i
	

!	Print*, k
		end do

	Print*, "NEW LCC will be", Minur
	Print*, "Z vaule", kr/100	
