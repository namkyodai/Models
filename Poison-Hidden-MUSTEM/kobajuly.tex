...
日常的な道路巡回を通じて、ポットホール等の局所的損傷に対する補修を行うと同時に、数年程度の間隔を経て路面性状調査し
、オーバレイや打ちかえ等の大規模な補修が実施される。近年、舗装劣化の統計的予測方法\cite{weibull}${}^{-}$\cite{hu}が
提案され、ポットホール等の日常的点検・補修政策の合理化や、舗装の劣化予測手法と連動したライ
フサイクル費用評価手法\cite{otaz1}${}^{,}$\cite{kait1}も提案されている．

既往研究では、ポットホール等の損傷発生と舗装の劣化過程の間には相互関係が存在する可能性がある。現実には、
舗装にひび割れが発生すれば、ポットホールに進展する場合が少なくない。このように局所的な舗装の損傷発生が、舗装全体の健全度
...
$\pi(y_{u_n}|l_{u_n})$は
\begin{eqnarray}
\pi(y_{u_n}|l_{u_n}) &=& \mbox{Prob}[g(u_n)=y_{u_n}|h(t_n+u_n)=l_{u_n}] \nonumber \\
...
\begin{eqnarray}
&& \tilde{\pi}_{u_n}(y_{u_n}) = \sum_{l_{u_n}=i}^j \pi(y_{u_n}|l_{u_n}) \rho_{u_n}(l_{u_n}|i)  
\end{eqnarray}
と定義される．ここで，路面性状調査時点$t_n,t_{n+1}$における健全度$h(t_n)=i$の下で，局所時
点$u_n~(u_n=0,\cdots,T_n-1)$における
ポットホール数の観測値ベクトル$\bar{\mbox{\boldmath$y$}}_n=(\bar{y}_0,\cdots,\bar{y}_{T_n-1})$と時点$t_{n+1}$
...
\begin{manyeqns}
&& {\cal L}(\bar{\xi}_n,\mbox{\boldmath$\theta$}) = \pi(\bar{y}_0|i) \sum_{l_1=i}^j p_{il_1}\ell_{1}(l_1) \label{mu0}\\
&& \ell_{u_n}(l_{u_n})= \pi(\bar{y}_{u_n}|l_{u_n}) \nonumber \\
&& \hspace{10mm} \sum_{l_{u_n+1}=l_{u_n}}^j p_{l_{u_n}l_{u_{n}+1}} \ell_{u_n+1}(l_{u_n+1}) \\
&& (1\leq u_n \leq T_n-1) \nonumber \\
&& \ell_{T_{n}-1}(l_{T_n-1})= \pi(\bar{y}_{T_n-1}|l_{T_n-1}) p_{l_{T_n-1}j}  \label{mu1}
\end{manyeqns}
と定義できる．
...
において道路巡回が実施され，ポットホール個数ベクトル$\bar{\mbox{\boldmath$y$}}_n^k=(\bar{y}_{0}^k,\cdots,\bar{y}_{T_n-1}^k)$が観測された
と考える．この時，道路区間$k$の期間$\tau_n$のそれぞれに対してデータ$\bar{\xi}_n^k=\{\bar{\mbox{\boldmath$y$}}_n^k,\bar{h}_k(t_n),\bar{h}_k(t_{n+1})\}$
...
多くの解を有している点にある\cite{titter}．当然のことながら，ポットホールの発生確率$\pi^k(\bar{y}^k_{u_n}|l_{u_n}^k)$, 
推移確率$p^k_{l_{u_n}^kl_{u_{n+1}}^k}$の推定値は実数解でなければならない．さらに，
...
\pi^k(\bar{y}_{u_n}|s^k_{u_n})^{\delta_{s_{u_n}}^k} \{p_{s^k_{u_n}s^k_{u_n+1}}\}^{\delta^k_{s_{u_{n}}}} 
\nonumber \\
&&= \prod_{u_n=0}^{T_n-1} \pi^k(\bar{y}_{u_n}|\tilde{m}^k_{u_n}) p_{\tilde{m}^k_{u_n}\tilde{m}^k_{u_{n}+1}} 
 \label{hyuudo2}
\end{eqnarray}
...
&& =\frac{
\pi^k(\bar{y}_{u_n}^k|m) \omega_{m,t}^k(\tilde{m}_{u_{n}-1}^k,\tilde{m}_{u_{n}+1}^k) }{
\sum_{m=\tilde{m}_{u_n-1}^k}^{\tilde{m}_{u_n+1}^k} \pi^k(\bar{y}_{u_n}^k|m) \omega_{m,t}^k(\tilde{m}_{u_{n}-1}^k,\tilde{m}_{u_{n}+1}^k) } \label{hhu}
\end{eqnarray}
と表される．ただし，
\begin{eqnarray}
...
\right.
\end{eqnarray}
である。すなわち，道路区間$k$におけるポットホールの発生確率$\pi^k(\bar{y}_{u_n}^k|m)$と推移確率$p^k_{m\tilde{m}_{u_{n}+1}^k}$~$(u_n=0,\cdots,T_n-1;n=1,\cdots,N,k=1,\cdots,K)$
 ...
 %
   \begin{eqnarray}
      ...
      && \propto \prod_{k=1}^K \prod_{n=1}^{N} \prod_{u_n=0}^{T_n-1} \left[
            \exp\left(- \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^k} \right)\left(
      \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^k}\right)^{\bar{y}_{u_n}^k} \right.\nonumber \\
...
&& \prod_{k=1}^K \prod_{n=1}^{N} \prod_{u_n=0}^{T_n-1} \left[
            \exp\left(- \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^k} \right)\left(
      \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^k}\right)^{\bar{y}_{u_n}^k} \prod_{i=1}^{I-1} \right.\nonumber \\
      && 
      \exp\Big\{-\frac{1}{2}(\mbox{\boldmath$\alpha$}^i
      -\mbox{\boldmath$\zeta$}^{i,\alpha})
      \{\mbox{\boldmath$\Sigma$}^{i,\alpha}\}^{-1}
...
&& \left\{ \exp\left(- \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^{k(v-1)}(v)} \right)\left(
      \mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^{k(v-1)}(v)}\right)^{\bar{y}_{u_n}^k}\right\}^{\delta_{i}^{u_nk}}
\nonumber \\
...
&&  \left\{\exp \left(- z_p^k \alpha_p^{\tilde{m}_{u_n}^{k(v-1)}(v)} \right)
\left(\mbox{\boldmath$z$}^k\mbox{\boldmath$\alpha$}^{\tilde{m}_{u_n}^{k(v-1)}(v)}\right)^{\bar{y}_{u_n}^k}\right\}^{\delta_{i}^{u_nk}} \nonumber \\
...
\bar{\mbox{\boldmath$\xi$}}\} \nonumber \\
&& =\frac{
\pi^k(\bar{y}_{u_n}^{k(v)}|m) \omega_{m,u_n}^k(\tilde{m}_{u_{n}-1}^{k(v)},\tilde{m}_{u_{n}+1}^{k(v-1)}) }{
\sum_{m=\tilde{m}_{u_n-1}^{k(v)}}^{\tilde{m}_{u_n+1}^{k(v-1)}} \pi^k(\bar{y}_{u_n}^{k(v)}|m) \omega_{m,u_n}^k(\tilde{m}_{u_{n}-1}^{k(v)},\tilde{m}_{u_{n}+1}^{k(v-1)}) } \nonumber \\
&& \label{hhu}
\end{eqnarray}
と表される。ただし、
\begin{eqnarray}
