...
*****************************************************************
	Module variable
	integer, parameter:: totaldata=10000

	real(8), dimension(totaldata):: d, t, le
	integer, parameter:: timemax=1000000 !000000
	
	character(len=19), parameter:: file1="input\typeA.dat" !
...
!	real*8, dimension(2):: ts
	...
	Print*, "Optimal value ===alpha =", ts(1)
	Print*, "Optimal value =======m =", ts(2)
	alpha=ts(1)
	m=ts(2)
...
!------------------------------------------------
		df(1)=df(1) + d(k)/ts(1) - (1.0/le(k))*t(k)**ts(2)
		!with respect to alpha
		df(2)=df(2)+ d(k)*(1.0/ts(2)+
     #		log(t(k)))-(1.0/le(k))*ts(1)*ts(2)*(t(k)**(ts(2)-1))
	    ! with respect to m
	! Formula for the second  derivative of Objective likihood function
	
	hes(1,1)=hes(1,1)-d(k)/(ts(1)**2)

	hes(1,2)=hes(1,2)-(1.0/le(k))*ts(2)*(t(k)**(ts(2)-1))

	hes(2,1)=hes(2,1)-(1.0/le(k))*ts(2)*(t(k)**(ts(2)-1))

	hes(2,2)=hes(2,2)-d(k)/(ts(2)**2)-(1.0/le(k))*
     #	ts(1)*(t(k)**(ts(2)-1)+ts(2)*(ts(2)-1)* (t(k)**(ts(2)-2)))
!-------------------------------------------------		
		end do
	end do
...
!	ts(2)=x(2)
	...
	do k=1, totaldata
		obj=obj+(1-d(k))*(-alpha*1/le(k))*t(k)**m  
     #	+ d(k)*(log(alpha*1/le(k))+log(m)+(m-1.0)
     #    *log(t(k))-(alpha*1/le(k))*t(k)**m)
	end do
	end function obj
