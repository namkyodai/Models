...
*****************************************************************
	Module variable
	integer, parameter:: totaldata=1952

	real(8), dimension(totaldata):: d, t, le, x0, x1
	integer, parameter:: timemax=1000000 !000000
	
	character(len=19), parameter:: file1="input\C.csv" !
...
***********************************************************************	
	...
	real*8:: m, eps, obj, c1, c2
	real*8:: alpha1, alpha2, alpha3, alpha0
	real*8:: hess(3,3), ts(3), optts(3), df(3), grad(3), tv(3)
!	real*8, dimension(2):: ts
	...
	open(10, file=file1)
	do k=1, totaldata
		read(10,*) t(k), d(k), x0(k), x1(k)
	!Print*, x0(k), x1(k), x2(k), x3(k)
	end do
	close(10)
...
!	m=2.391
	...
	Print*, "Optimal value ===alpha0 =", ts(1)
	Print*, "Optimal value ===alpha1 =", ts(2)
	Print*, "Optimal value =======m =", ts(3)
	...
	! This subroutine is used to estimate the t-value of alpha and m

	open(20, file=file2)
	
	Write(20,*) "=====value of alpha0 =", alpha0
	Write(20,*) "=====value of alpha1 =", alpha1
	Write(20,*) "=========value of m =", m
	
	call Newton(ts,hess,df)
...
!	c1=3687235 ! this is social cost
!	c2=2485511 ! this is direct repairing cost
!	r=0.002 ! this is interest rate

!	call renewal(alpha,m,c1,c2,r)
...
!	This subroutine only takes alpha and m into account (do not consider yet the length variable)
	...
	real*8, dimension(3):: ts, df, grad, hs, tv, ms, ss, mms, c, optts
	real*8, dimension(3,3):: hes, hess 

	real*8:: alpha0, m, eps, m0, alpha00, maxS, ts10, ts20
	real*8:: alpha1 , ts30, ts40, ts50
	integer:: i,j,k, time
...
!------------------------------------------------
		df(1)=df(1) + d(k)/(ts(1)*x0(k)) - t(k)**ts(3)
		!with respect to alpha0
	
	df(2)=df(2)+d(k)*x1(k)/(ts(1)*x0(k)+ts(2)*x1(k))-x1(k)*t(k)**ts(3)
		!with respect to alpha1
	
		df(3)=df(3)+ d(k)*(1.0/ts(3)+
     #	log(t(k)))-(ts(1)*x0(k)+ts(2)*x1(k))*ts(3)*(t(k)**(ts(3)-1.0))
	    ! with respect to m
...
!--------------------------------------------------------------------------	
	...
	hes(1,1)=hes(1,1)-d(k)/((ts(1)*x0(k)+ts(2)*x1(k))**2) ! ok

	hes(1,2)=hes(1,2)-d(k)*x1(k)/((ts(1)*x0(k)+ts(2)*x1(k))**2) !ok

	hes(1,3)=hes(1,3)-ts(3)*(t(k)**(ts(3)-1.0)) ! ok
!_______________________________________
	hes(2,1)=hes(1,2)

	hes(2,2)=hes(2,2)-d(k)*(x1(k)**2)/((ts(1)*x0(k)+ts(2)*x1(k))**2) !ok

	hes(2,3)=hes(2,3)- x1(k)*ts(3)*(t(k)**(ts(3)-1.0)) ! ok
!-------------------------------------------
	hes(3,1)=hes(1,3)
	hes(3,2)=hes(2,3)
	hes(3,3)=hes(3,3)-d(k)/(ts(3)**2)-(ts(1)*x0(k)+ts(2)*x1(k))*
     #	*ts(3)*(ts(3)-1.0)*(t(k)**(ts(3)-2))
...
400	continue
	...
	tv(3)=ts(3)/(sqrt(-hess(3,3)))

	Print*, "T-value"

	Print*, "T-value for alpha0", tv(1)
	Print*, "T-value for alpha1", tv(2)
	Print*, "T-value for m", tv(3)
	open(12, file='output\t-value.text')
!	Write(12,*) "=====t-value========="
	write(12,*) "t- for alpha0=", tv(1)
	write(12,*) "t- for alpha1=", tv(2)
	write(12,*) "===t- for  m=", tv(3)
...
!----------------------------------------------------------------------
!	All No. of variables is considered
!	do k=1, totaldata
!		obj=obj+ d(k)*(log(m) + (m-1.0)*log(t(k))+
 !    #	log(alpha0*x0(k)+alpha1*x1(k)+alpha2*x2(k)+alpha3*x3(k)))
  !   #	- (alpha0*x0(k)+alpha1*x1(k)+alpha2*x2(k)+alpha3*x3(k))
   !  #      *t(k)**m
     
!	end do
!------------------------------------------------------------------------
*	Only when 1 variable is considered
	do k=1, totaldata
		obj=obj+ d(k)*(log(m) + (m-1.0)*log(t(k))+
...
!	Subroutine for estimating the life cycle cost and optimal value of renewal time
	...
	subroutine renewal(alpha,m,c1,c2,r)
	implicit none
