...
!***********************************************************************
	...
	dimension beta(10,10),ep(idim0),ep0(idim0),ep00(idim0)
      character*20 file1,file2,diff0,cdif(idim0)!diff0 is number heterogeneity Number
	dimension ts(30)
...
c
      ...
      itotal=2063 !Data number
      timemax=100000!It is a calculation repeatedly frequency.
c
...
c	!Difference of each Θi-Θj
  ...
  320				continue
				sum1=sum1+product
     #                 *((phi**phi)/((theta(l)*zk(kth,ith)+phi)**phi))
c
  300			continue
			if (sum1.gt.0) then
				sum2=sum2+dlog(sum1)
